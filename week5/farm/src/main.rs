...
fn pop_number_from_queue(queue: &mut Arc<Mutex<VecDeque<u32>>>) -> Option<u32> {
    let mut q = queue.lock().unwrap();
    q.pop_front()
}

fn run(mut numbers: Arc<Mutex<VecDeque<u32>>>) {
    loop {
        if let Some(number) = pop_number_from_queue(&mut numbers) {
